...
export const WRITES_ENV = "BLOCKS_CONSOLE_ALLOW_WRITES";
...
export function createMcpServer(): McpServer {
  const server = new McpServer({ name: "blocks-console", version: "0.1.0" });
  const writesEnabled = process.env[WRITES_ENV] === "1";
  ...
  server.registerTool(
    ...
    async ({ project }) => {
      ...
      return asText({
        ...
        ...(writesEnabled
          ? {}
          : { writesNote: `Write tools are not registered. Set ${WRITES_ENV}=1 to enable them.` }),
      });
    },
  );
