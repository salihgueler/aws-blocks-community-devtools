...
import { resolveProject, type ProjectResolution } from "@aws-blocks-devtools/core";
import { detectEnvironment } from "@aws-blocks-devtools/core";
 ...
 *   aws-blocks-console connect    register the MCP server with your agents
 *   aws-blocks-console doctor     report what was detected, change nothing
 *
 * Running the MCP server is deliberately NOT a subcommand here: it ships as its
 * own package (aws-blocks-mcp) with its own bin, so an agent can spawn it
 * without installing a React UI it will never render.
 */
...
async function doctor(
  ...
  console.log(`agent writes   ${process.env[MCP_WRITES_ENV] === "1" ? "enabled" : `disabled (set ${MCP_WRITES_ENV}=1)`}`);
}
...
function printHelp(): void {
  console.log(`aws-blocks-console — inspect and interact with an AWS Blocks project

Usage
  aws-blocks-console [serve] [options]   Serve the console UI (default)
  aws-blocks-console connect [--local]   Register the MCP server with your agents
  aws-blocks-console doctor              Report what was detected
...
Environment
  ${MCP_WRITES_ENV}=1   Register write tools on the MCP server`);
}
